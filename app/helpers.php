...
{
    //return getUserImagenId($userId);
    return getAvatarUrl()."?name=$name&size=100&rounded=true&color=fff&background=".getRandomColor($userId);
}

/*function getUserImagenId($userId){
    $media=DB::table('media')->where( ['model_type' =>'App\Models\User', 'model_id'=>$userId])->first(); 
    if(is_null($media)){
...
function getMenuLinks($menu) {
    ...
    if ($menu == User::MAIN_DIAGNOSIS) {
        $defaultRoute = route('patient.diagnosis.test.index');
...
//    Front Office
    ...
    if ($menu == User::MAIN_PATIENT_CASE) {
        ...
        $subMenus = ['Patients', 'Cases', 'Case Handlers'];
    }
    // Pathology
    ...
    if ($menu == User::MAIN_RADIOLOGY) {
        $defaultRoute = route('radiology.test.index');
        $subMenus = ['Radiology Categories', 'Radiology Tests'];
    }
    // Service
    if ($menu == User::MAIN_SERVICE) {
        $defaultRoute = route('insurances.index');
        $subMenus = ['Packages', 'Services', 'Ambulances', 'Ambulance Calls'];
    }
    // Insurance
    if ($menu == User::MAIN_INSURANCE) {
        $defaultRoute = route('insurances.index');
        $subMenus = ['Insurances', 'Patient Admissions', 'Ambulances', 'Ambulance Calls'];
    }
    // Sms/Mail
